...
export default function Todo({ todo }) {
  ...
  function handleUpdateConfirm() {
    const updatedTodos = todosList.map((t) =>
      t.id === todo.id ? { ...t, title: updateInput, details: updateDetails } : t
    );
    setTodosList(updatedTodos);
    setUpdate(false);
  }
  ...
  return (
    <>
      ...
      <Dialog open={open} onClose={() => setOpen(false)}>
        ...
        <DialogActions>
          <Button style={{ color: 'red' }} onClick={() => setOpen(false)}>
            Close
          </Button>
      ...
      <Dialog open={update} onClose={() => setUpdate(false)}>
        <DialogTitle style={{ color: 'black' }}>Update Task</DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            required
        ...
        <DialogActions>
          <Button style={{ color: 'red' }} onClick={() => setUpdate(false)}>
            Cancel
          </Button>
          <Button style={{ color: 'green' }} onClick={handleUpdateConfirm}>
            Save
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Card
        className="todoCard"
        sx={{ minWidth: 275, backgroundColor: '#0016c0ff', color: 'white', marginTop: 5 }}
      >
        <CardContent>
          <Grid container spacing={2}>
            <Grid size={8}>
              <Typography gutterBottom variant="h5" sx={{ textAlign: 'left',textDecoration: todo.isCompleted ? 'line-through' : 'none' }}>
                {todo.title}
              </Typography>
              <Typography gutterBottom variant="h6" sx={{ textAlign: 'left',textDecoration: todo.isCompleted ? 'line-through' : 'none' }}>
                {todo.details}
              </Typography>
            </Grid>
